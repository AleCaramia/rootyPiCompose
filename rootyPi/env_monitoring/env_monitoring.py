...
class EnvMonitoring(object):
    ...
    def PlantLuxEstimation(self,url_adptor,plant_code,plant_type,userId,hours_passed,suncycle,max_lux_lamp,lamp_intensity):               
        ...
        mesurements_past_hour,DLI_daily_record=self.PlantSpecificGetReq(url_adptor,userId,plant_code,hours_passed)#aggiungo i dli dati fin ora
        #ipotizzando siano lux al secondo e siano in un vettore
        # lux_past_hour=self.LuxPastHour(mesurements_past_hour)
        lux_past_hour,lamp_intensity,dli_given_today=self.LuxPastHour(mesurements_past_hour,lamp_intensity,DLI_daily_record)  
        current_plant_goal=self.goals(plant_type)

        lux_to_add,dli_recived_past_hour=self.LuxDeficitCalculation(current_plant_goal,suncycle,lux_past_hour,lamp_intensity,dli_given_today,hours_passed,max_lux_lamp)
    ...
    ###################################################################################
        return mesurements_past_hour,DLI_daily_record

    def LuxPastHour(self,mesurments_past_hour,lamp_intensity,DLI_daily_record):
        lux_past_hour_v=[]
        # lamp_past_hour_v=[]
        dli_given_today=0
        ...
        #     lamp_past_hour_v.append(mesure['v'])
        ...
        return lux_past_hour,dli_given_today
    ...
    def LuxDeficitCalculation(self,DLI_goal,sun_cycle,registered_Lux_hour,lamp_intensity,dli_given_today,hours_passed,max_lux_lamp):
    ...
    #maxLuxLamp: How many lux can emit the SPECIFIC LAMP
        ...
        PPFDlamp=max_lux_lamp*(lamp_intensity/100)*0.0135 #the factor depends on kind of lamp
        #print(PPFDlamp)
        PPFDsun=(registered_Lux_hour-(max_lux_lamp*lamp_intensity/100))*0.0185
        #print(PPFDsun)
...
###################################################################################################################################################
# class EnvMonitoringTime(object):
#     exposed=True
#     def __init__(self):
#         self.envTime="EnviromentMonitoring\envTime.json"
#         self.default_start="08:00"
#         self.default_end="20:00"
#         self.format={"user":"","plants":[{"plantId":"","start_time":"","end_time":""}]}
#         self.mess={"start_time":'',"end_time":''}
#         pass

#     def GET(self,*uri,**params):
#         if len(uri)>0 and str(uri[0])=="GetEnvTime":
#             user=params['user']
#             plant=params['plant']
#             current_time=json.load(open(self.envTime,'r'))
#             res=0
#             for listed in current_time:
#                 reg_user=listed["user"]
#                 if user == reg_user:
#                     for listed_plant in listed["plants"]:
#                         reg_plant=listed_plant['plantId']
#                         if plant == reg_plant:
#                             start_time=listed_plant["start_time"]
#                             end_time=listed_plant["end_time"]
#                             res=1
#                             break
#             if res==0:
...
#         if len(uri)>0 and str(uri[0])=="ChangeEnvTime":
...
#                         reg_plant=listed_plant['plantId']
#                         if plant == reg_plant:
#                             listed_plant['start_time']=new_start
#                             listed_plant['end_time']=new_end
#                             res=2
#                             break
#                     if res==1:
#                         new=self.format["plants"][0].copy()
#                         new['plantId']=plant
#                         new['start_time']=new_start
#                         new['end_time']=new_end
#                         listed['plants'].append(new)
#                         break
#             if res==0:
#                 new=self.format.copy()
#                 new["user"]=user
#                 new["plants"][0]['plantId']=plant
#                 new["plants"][0]["start_time"]=new_start
#                 new["plants"][0]["end_time"]=new_end
#                 current_time.append(new)

#             with open(self.envTime, 'w') as file:
#                 json.dump(current_time, file, indent=4)
...
class Iamalive(object):
    ...
    def __init__(self,settings):
        ...
        self.clientID = f"{self.base_topic}/EnvMonitoring"       
        ...
        self.message = {"n": "EnvMonitoring", "t": time.time(), "v":"env_monitoring"}
        #########################################################################################à
        self.time = time.time()
    ...
    def myconnect(self,paho_mqtt, userdata, flags, rc):
       print(f"Env monitoring: Connected to {self.broker} with result code {rc} \n subtopic {None}, pubtopic {self.pub_topic}")
